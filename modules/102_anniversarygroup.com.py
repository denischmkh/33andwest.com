...
url = f"https://anniversarygroup.com/ArtistListView"
website_link = 'https://anniversarygroup.com'
agency_name = 'anniversarygroup.com'
...
if response.status_code == 200:
    ...
    artists = soup.find_all("div",attrs={"grid-gutter":"5"})[1]
    div_all = artists.find_all("div")
    for div in div_all:
        small_all = div.find_all("small")
        for smal in small_all:
            a_all = smal.find_all("a")
            for art in a_all:
                text = art.text.strip()
                if text:
                    #print(f"Name: {text}")
