...
url = f"https://www.ayita.com/"
website_link = 'https://www.ayita.com'
agency_name = 'ayita.com'
...
def parse68():
    ...
    if response.status_code == 200:
        ...
        artists = soup.find_all("p",attrs={"data-rte-preserve-empty":"true"})
        for a in artists:
            a_all = a.find_all("a")
            for art in a_all:
                text = art.text.strip()
                if text == "A CIRCUIT GROUP COMPANY":
                    text = None
                    break
                else:
                    #print(f"Name: {text}")
